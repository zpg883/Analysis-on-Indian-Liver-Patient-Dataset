...
```
```{r}
(inf.id.M = which(cooks.distance(stepwise.Male)>0.25))

glm.male.final = glm(LiverPatient ~ DB + Alamine + Age + Alkphos, data = liverMale[-inf.id.M, ], family = "binomial")
```
...
```
```{r}
(inf.id=which(cooks.distance(step.sleep1)>0.25))
```
...
```{r}
glm.sleep.final = glm(maxlife10 ~ brainweight + totalsleep + as.factor(predationindex) + as.factor(sleepexposureindex), data = sleep[-inf.id], family = binomial)
